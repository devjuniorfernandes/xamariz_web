...
@php
    ...
    $srvTagline = is_object($service) ? ($service->tagline ?? $service->short_description) : 'Transformamos mensagens corporativas complexas em posicionamentos claros e líderes no mercado.';
    $srvDesc = is_object($service) ? ($service->full_description ?? $service->short_description) : 'O Marketing 360° da Xamariz conecta todos os pontos de contacto da sua empresa ao seu público-alvo em Angola e no mercado internacional.';
    
    $imgPath = is_object($service) ? $service->image_path : null;
    $srvImage = $imgPath ? (Str::startsWith($imgPath, 'http') ? $imgPath : asset($imgPath)) : 'https://images.unsplash.com/photo-1557804506-669a67965ba0?w=1800&auto=format&fit=crop&q=80';
    ...
    $deliverablesList = is_object($service) && !empty($service->deliverables) ? $service->deliverables : [
    ...
    $methodologyList = is_object($service) && !empty($service->methodology) ? $service->methodology : [
    ...
    $metricsList = is_object($service) && !empty($service->metrics) ? $service->metrics : [
...
@section('title', $srvTitle . ' | Xamariz Marketing 360°')
@section('description', Str::limit(strip_tags($srvDesc), 160))
    ...
    <section class="pt-40 pb-20 bg-white text-gray-900 border-b border-gray-100">
        <div class="container-myriad">
            ...
            <div class="reveal flex items-center gap-3 text-xs uppercase font-sans tracking-widest mb-6 sm:mb-8">
                <a href="{{ route('services.index') }}" class="text-gray-600 hover:text-gray-900 transition-colors">Serviços</a>
                <span class="text-gray-300">/</span>
                <span class="text-gray-400 truncate max-w-xs sm:max-w-md">{{ $srvTitle }}</span>
            </div>

            <div class="max-w-4xl reveal">
                <h1 class="font-sans text-4xl sm:text-6xl md:text-7xl font-bold text-gray-900 tracking-tight leading-[1.1] mb-6">
                    {{ $srvTitle }}.
                </h1>
                @if($srvTagline)
                    <p class="font-sans text-xl sm:text-2xl font-semibold text-[var(--color-brand-accent)] leading-relaxed mb-6">
                        {{ $srvTagline }}
                    </p>
                @endif
                <p class="font-sans text-gray-600 text-base sm:text-lg leading-relaxed mb-10 max-w-3xl">
                    {{ $srvDesc }}
                </p>
                ...
                <div class="flex flex-wrap items-center gap-4">
                    <a href="{{ route('contact') }}"
                        class="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-[var(--color-brand-accent)] text-white hover:bg-[var(--color-brand-accent-hover)] text-xs font-semibold uppercase tracking-wider transition-all duration-300 group">
                    ...
                    <a href="{{ route('work.index') }}"
                        class="inline-flex items-center gap-3 px-8 py-4 rounded-full border border-gray-900 text-gray-900 hover:border-[var(--color-brand-accent)] hover:text-[var(--color-brand-accent)] text-xs font-semibold uppercase tracking-wider transition-all duration-300 group">
            ...
            <div class="container-myriad">
                <div class="max-w reveal space-y-8">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-700">VALOR ESTRATÉGICO</span>
                    </div>
                    ...
                    {{-- Caixa de Citação --}}
                    @if($quoteText)
                        <div class="p-8 border-l-4 border-[var(--color-brand-accent)] bg-gray-50 text-gray-900 rounded-none mt-8">
            ...
            <div class="container-myriad">
                ...
                <div class="reveal max-w-3xl mb-16">
                    <div class="flex items-center gap-2 mb-3">
                        <span class="w-2.5 h-2.5 rounded-full bg-[var(--color-brand-accent)] inline-block"></span>
                        <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-700">O QUE ENTREGAMOS</span>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 reveal">
                    @foreach ($deliverablesList as $index => $item)
                        <div class="p-8 bg-white border border-gray-200 rounded-none hover:border-[var(--color-brand-accent)] transition-all duration-300 group flex flex-col justify-between">
                            <div>
                                <div class="flex items-center justify-between mb-6">
                                    <span class="font-mono text-xs font-bold text-[var(--color-brand-accent)]">0{{ $index + 1 }}</span>
                                    <span class="w-2 h-2 rounded-full bg-[var(--color-brand-accent)] inline-block"></span>
                                </div>
                                <h3 class="font-sans text-xl font-bold text-gray-900 mb-3 tracking-tight group-hover:text-[var(--color-brand-accent)] transition-colors">
                                    {{ $item['title'] ?? '' }}
                                </h3>
        ...
        <section class="py-24 bg-white">
            <div class="container-myriad">
                ...
                <div class="reveal max-w-3xl mb-16">
                    <div class="flex items-center gap-2 mb-3">
                        <span class="w-2.5 h-2.5 rounded-full bg-[var(--color-brand-accent)] inline-block"></span>
                        <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-700">METODOLOGIA XAMARIZ</span>
                    </div>
                    <h2 class="font-sans text-3xl sm:text-5xl font-bold text-gray-900 tracking-tight">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 reveal">
                    @foreach ($methodologyList as $index => $process)
                        <div class="p-8 border-l-2 border-gray-200 hover:border-[var(--color-brand-accent)] transition-colors duration-300">
                            <span class="font-mono text-xs font-bold text-[var(--color-brand-accent)] tracking-widest block mb-3">PASSO {{ $process['step'] ?? ('0' . ($index + 1)) }}</span>
                            <h3 class="font-sans text-lg font-bold text-gray-900 mb-2 tracking-tight">{{ $process['title'] ?? '' }}</h3>
            ...
            <div class="container-myriad">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-12 text-center reveal">
                    @foreach ($metricsList as $metric)
                        <div class="space-y-2">
                            <span class="font-sans text-4xl sm:text-6xl font-bold text-[var(--color-brand-accent)] tracking-tight block">{{ $metric['value'] ?? '' }}</span>
                            <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 block">{{ $metric['label'] ?? '' }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="container-myriad">
                <div class="reveal flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
                    <div>
                        <div class="flex items-center gap-2 mb-3">
                            <span class="w-2.5 h-2.5 rounded-full bg-[var(--color-brand-accent)] inline-block"></span>
                            <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-700">EXPLORAR MAIS</span>
                        </div>
                        <h2 class="font-sans text-3xl sm:text-5xl font-bold text-gray-900 tracking-tight">
                            Outros Serviços 360°
                        </h2>
                ...
                <div class="space-y-6 reveal">
                    @foreach ($allServices as $otherService)
                        @if (is_object($service) ? ($otherService->id !== $service->id) : ($otherService->slug !== $slug))
                            <div class="p-8 sm:p-10 border border-gray-200 bg-gray-50/50 hover:bg-gray-50 transition-colors rounded-none">
                                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                                    <div class="space-y-4 max-w-3xl">
                                        <div class="flex items-center gap-3">
                                            <span class="font-mono text-xs font-bold text-[var(--color-brand-accent)] uppercase tracking-widest">{{ $otherService->number_code }}</span>
    ...
    <section class="py-24 bg-[var(--color-brand-dark)] text-white">
        <div class="container-myriad text-center reveal">
            <div class="inline-flex items-center gap-2 mb-4">
                <span class="w-2.5 h-2.5 rounded-full bg-[var(--color-brand-accent)] inline-block"></span>
                <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-400">PRONTO PARA CRESCER?</span>
            </div>
            <h2 class="font-sans text-3xl sm:text-5xl font-normal text-white mb-6 tracking-tight">
                Transforme a sua mensagem em liderança de mercado.
            </h2>
            <p class="font-sans text-gray-400 text-lg mb-10 max-w-md mx-auto leading-relaxed">
                Fale com a nossa equipa de especialistas e descubra a melhor estratégia para o seu negócio.
            </p>
            <a href="{{ route('contact') }}"
                class="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-[var(--color-brand-accent)] text-white hover:bg-[var(--color-brand-accent-hover)] text-xs font-semibold uppercase tracking-wider transition-all duration-300 group">
