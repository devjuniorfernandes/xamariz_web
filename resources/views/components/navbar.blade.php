@php
    ...
    $navLinks = [
        ...
        ['label' => 'Serviços',  'pattern' => 'services.*', 'name' => 'services.index', 'url' => route('services.index')],
        ...
        ['label' => 'Insights',  'pattern' => 'insights.*', 'name' => 'insights.index', 'url' => route('insights.index')],
    ];
...
<nav
    ...
    :class="mobileOpen ? 'fixed inset-0 w-full h-full min-h-screen bg-white z-[999999] overflow-y-auto' : ((isDarkNav && !scrolled) ? 'fixed top-0 left-0 right-0 bg-transparent border-b border-transparent z-[999999] transition-all duration-300' : 'fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm z-[999999] transition-all duration-300')"
>
        ...
        <div class="flex items-center justify-between h-[72px]">
            ...
            <a href="{{ route('home') }}" class="flex items-center shrink-0 group">
                <template x-if="isDarkNav && !scrolled">
                    <img 
                        src="{{ asset('logo_xamariz_white.svg') }}" 
                        alt="Xamariz" 
                ...
                <template x-if="!(isDarkNav && !scrolled)">
                    <img 
                        src="{{ asset('logo_xamariz.svg') }}" 
                        alt="Xamariz" 
                        class="hidden md:block h-8 w-auto object-contain transition-transform duration-300 group-hover:scale-105"
                    >
                </template>
                <img 
                    src="{{ asset('logo_xamariz_mobile.svg') }}" 
                    alt="Xamariz" 
            ...
            <div class="hidden md:flex items-center gap-1.5 h-full">
                @foreach ($navLinks as $link)
                    ...
                    <a
                        href="{{ $link['url'] }}"
                        :class="(isDarkNav && !scrolled) ? '{{ $isActive ? 'text-white font-bold' : 'text-white/80 hover:text-white' }}' : '{{ $isActive ? 'text-gray-900 font-bold' : 'text-gray-700 hover:text-gray-900' }}'"
                    ...
                    >
                        ...
                        @else
                            <span class="w-1.5 h-1.5 rounded-full bg-[var(--color-brand-accent)] absolute bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-60 transition-all duration-200"></span>
                        @endif
                    </a>
                @endforeach
            </div>
            ...
            <div class="flex items-center gap-3">
                <a
                    href="{{ route('contact') }}"
                    :class="(isDarkNav && !scrolled) ? 'border-white/30 text-white hover:bg-white hover:text-[var(--color-brand-dark)]' : 'border-[var(--color-brand-dark)] text-[var(--color-brand-dark)] hover:bg-[var(--color-brand-dark)] hover:text-white'"
    ...
    <div
        ...
        class="w-full min-h-screen bg-white text-gray-900 flex flex-col justify-between p-6 sm:p-8"
        x-cloak
    >
        <div>
            ...
            <div class="flex items-center justify-between pb-6 border-b border-gray-100">
                <a href="{{ route('home') }}" @click="mobileOpen = false">
                    <img src="{{ asset('logo_xamariz_mobile.svg') }}" alt="Xamariz" class="h-10 w-auto object-contain">
                </a>
                <button @click="mobileOpen = false" class="p-2 text-gray-700 hover:text-[var(--color-brand-accent)] transition-colors focus:outline-none" aria-label="Fechar Menu">
        ...
        <div class="pt-8 pb-4 flex items-center justify-between gap-2 border-t border-gray-100 mt-8">
            <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" 
                class="w-12 h-12 rounded-full bg-gray-100 hover:bg-[var(--color-brand-accent)] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-200" aria-label="Facebook">
                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
            </a>
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" 
                class="w-12 h-12 rounded-full bg-gray-100 hover:bg-[var(--color-brand-accent)] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-200" aria-label="Instagram">
                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
            ...
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" 
                class="w-12 h-12 rounded-full bg-gray-100 hover:bg-[var(--color-brand-accent)] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-200" aria-label="LinkedIn">
                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
            </a>
            <a href="https://x.com" target="_blank" rel="noopener noreferrer" 
                class="w-12 h-12 rounded-full bg-gray-100 hover:bg-[var(--color-brand-accent)] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-200" aria-label="X (Twitter)">
                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
            </a>
            <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" 
                class="w-12 h-12 rounded-full bg-gray-100 hover:bg-[var(--color-brand-accent)] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-200" aria-label="TikTok">
                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
