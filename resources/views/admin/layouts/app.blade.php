...
<head>
    ...
    <style>
        ...
        /* Campos de formulário: texto sempre escuro em light mode.
           (Necessário porque muitos inputs referem "brand-accent" no focus,
           o que os excluía do remapeamento do .text-white acima.) */
        [data-admin-theme="light"] input,
        [data-admin-theme="light"] textarea,
        [data-admin-theme="light"] select { color: #111827 !important; }
        [data-admin-theme="light"] input::placeholder,
        [data-admin-theme="light"] textarea::placeholder { color: #9199a5 !important; }
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'" class="fixed inset-y-0 left-0 z-50 w-72 bg-[#1f2022] border-r border-[#2c2d30] transition-transform duration-300 ease-in-out flex flex-col justify-between">
            <div>
                ...
                <nav class="px-4 py-6 space-y-1.5 overflow-y-auto max-h-[calc(100vh-140px)]">
                    ...
                    <a href="{{ route('admin.landing.index') }}" class="flex items-center gap-3 px-4 py-3 text-xs font-bold uppercase tracking-wider rounded-full transition-all {{ request()->routeIs('admin.landing.*') ? 'bg-[var(--color-brand-accent)] text-white' : 'text-gray-400 hover:text-white hover:bg-[#2c2d30]' }}">
