    ...
    <form action="{{ route('admin.works.update', $work) }}" method="POST" enctype="multipart/form-data" class="bg-[#1f2022] border border-[#2c2d30] rounded-none p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="summary" rows="3" required class="w-full px-4 py-3 rounded-none bg-[#141414] border border-[#2c2d30] text-white focus:outline-none focus:border-[var(--color-brand-accent)] text-sm">{{ old('summary', $work->summary) }}</textarea>
            </div>

            <div class="md:col-span-2">
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-300 mb-2">Descrição Completa / Case Study *</label>
                <textarea name="description" rows="6" required class="w-full px-4 py-3 rounded-none bg-[#141414] border border-[#2c2d30] text-white focus:outline-none focus:border-[var(--color-brand-accent)] text-sm">{{ old('description', $work->description) }}</textarea>
            </div>
