...
@section('title', 'Novo Serviço | CMS Xamariz')
@section('header_title', 'Adicionar Novo Serviço 360°')

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    deliverables: {{ json_encode(old('deliverables', [
        ['title' => '', 'desc' => ''],
        ['title' => '', 'desc' => '']
    ])) }},
    methodology: {{ json_encode(old('methodology', [
    ...
    metrics: {{ json_encode(old('metrics', [
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-bold text-white">Novo Serviço 360°</h2>
        <a href="{{ route('admin.services.index') }}" class="text-xs text-slate-400 hover:text-white">&larr; Voltar à lista</a>
    </div>

    <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="bg-slate-900 border border-slate-800/80 rounded-2xl p-8 space-y-8">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="number_code" value="{{ old('number_code', '01') }}" required class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white font-mono focus:outline-none focus:border-[#fe3d0a] text-sm">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Título do Serviço *</label>
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Ex: Gestão de Redes Sociais" class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">
                </div>

                <div class="flex items-center gap-3">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-700 bg-slate-950 text-[#fe3d0a] focus:ring-[#fe3d0a]">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="tagline" value="{{ old('tagline') }}" placeholder="Ex: Transformamos mensagens corporativas complexas em posicionamentos claros..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="short_description" rows="2" placeholder="Resumo em 2 linhas para os cards da homepage..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">{{ old('short_description') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="full_description" rows="3" placeholder="Explicação detalhada da oferta do serviço..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">{{ old('full_description') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Imagem de Destaque (Upload SVG / PNG / WEBP)</label>
                ...
                <div>
                    ...
                    <input type="url" name="image_url" value="{{ old('image_url') }}" placeholder="https://images.unsplash.com/..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6 pt-4 border-t border-slate-800">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="strategic_value_para1" rows="3" placeholder="Primeiro parágrafo detalhando o impacto de mercado..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">{{ old('strategic_value_para1') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="strategic_value_para2" rows="3" placeholder="Segundo parágrafo detalhando diferenciação e autoridade..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">{{ old('strategic_value_para2') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="quote" value="{{ old('quote') }}" placeholder="Ex: Um website moderno é a sede digital do seu negócio..." class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#fe3d0a] text-sm">
                </div>
            </div>
        </div>
                ...
                <template x-for="(item, index) in deliverables" :key="index">
                    <div class="p-4 bg-slate-950 border border-slate-800/80 rounded-xl space-y-3 relative">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <input type="text" :name="'deliverables['+index+'][title]'" x-model="item.title" placeholder="Título da Solução (Ex: Consultoria Estratégica)" class="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-white text-xs">
                            </div>
                            <div>
            ...
            <div class="space-y-4">
                <template x-for="(item, index) in methodology" :key="index">
                    <div class="p-4 bg-slate-950 border border-slate-800/80 rounded-xl space-y-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <input type="text" :name="'methodology['+index+'][title]'" x-model="item.title" placeholder="Título do Passo (Ex: Diagnóstico & Imersão)" class="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-white text-xs">
                            </div>
                            <div>
        ...
        <div class="pt-6 border-t border-slate-800 flex items-center justify-end gap-4">
            ...
            <button type="submit" class="px-6 py-3 rounded-full bg-[#fe3d0a] hover:bg-[#d63205] text-white text-xs font-bold uppercase tracking-wider shadow-lg shadow-[#fe3d0a]/25 transition-all">Guardar Serviço 360°</button>
        </div>
    </form>
