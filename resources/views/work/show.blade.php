...
@php
    $finalWorkTitle = is_object($work) && $work->meta_title ? $work->meta_title : $title . ($clientName ? ' • ' . $clientName : '') . ' | Portfólio Xamariz';
    $finalWorkDesc = is_object($work) && $work->meta_description ? $work->meta_description : Str::limit(strip_tags($summary ?: $description), 160);
    $finalWorkImg = $cover ? (Str::startsWith($cover, ['http://', 'https://']) ? $cover : asset($cover)) : \App\Models\SiteSetting::get('seo_og_image_default');
@endphp

@section('title', $finalWorkTitle)
@section('description', $finalWorkDesc)
@section('og_image', $finalWorkImg)
...
@section('content')
        ...
        <div class="container-myriad py-4 flex items-center justify-between gap-4">
            ...
            @if (!empty($externalUrl))
                <a href="{{ $externalUrl }}" target="_blank" rel="noopener noreferrer"
                    class="btn-primary py-2 px-5 text-xs inline-flex items-center gap-2">
        ...
        <div class="container-myriad">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
                ...
                <div class="lg:col-span-4 space-y-8">
                    ...
                    <div class="border-t border-gray-200 pt-6 space-y-4 reveal delay-100">
                        <div>
                            <p class="font-sans text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">CLIENTE
                            </p>
                        ...
                        @if (!empty($categoryName))
                            <div>
                                <p class="font-sans text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">
                                    SECTOR / FILTRO</p>
                        ...
                        @if (!empty($externalUrl))
                            <div class="pt-2">
                                <a href="{{ $externalUrl }}" target="_blank" rel="noopener noreferrer"
                                    class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[var(--color-brand-accent)] hover:underline">
                                    <span>Ver Projeto Concluído ↗</span>
                                </a>
                            </div>
                        @endif
                ...
                <div
                    class="lg:col-span-7 lg:col-start-6 space-y-6 font-sans text-gray-600 text-base sm:text-lg leading-relaxed">
                    <div class="reveal">
                        {!! nl2br(e($description)) !!}
                    </div>
            ...
            @if (!empty($embedVideoUrl))
                <div class="mt-16 reveal">
                    <h3 class="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">VÍDEO DO PROJETO /
                        SPOT PUBLICITÁRIO</h3>
                    <div class="aspect-video w-full overflow-hidden bg-black shadow-2xl">
                        @if (str_contains($embedVideoUrl, 'youtube.com') || str_contains($embedVideoUrl, 'vimeo.com'))
                            <iframe src="{{ $embedVideoUrl }}" class="w-full h-full border-0"
            ...
            @if (!empty($gallery) && count($gallery) > 0)
                <div class="mt-20">
                    <h3 class="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 mb-8">GALERIA DE IMAGENS
                        DO PROJETO</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
                        @foreach ($gallery as $i => $item)
                            ...
                            @if (!empty($imgUrl))
                                <div class="{{ $gridClass }} overflow-hidden group relative reveal"
                                    style="animation-delay: {{ $i * 100 }}ms">
                                    <img src="{{ $imgUrl }}" alt="Galeria {{ $title }} {{ $i + 1 }}"
                                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                                    @if (!empty($caption))
                                        <div
                                            class="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent text-white text-xs font-sans">
                                            {{ $caption }}
                                        </div>
                                    @endif
                                </div>
    ...
    {{-- CTA --}}
    <x-cta-section
        title="Pretende um projeto com este nível de impacto?"
        subtitle="Fale com a equipa de especialistas da Xamariz." />
