...
@section('title', \App\Models\SiteSetting::get('seo_team_title', 'A Nossa Equipa | Especialistas em Marketing 360° • Xamariz'))
@section('description', \App\Models\SiteSetting::get('seo_team_description', 'Conheça os estrategistas, criativos, contadores de histórias e especialistas em performance dedicados à excelência na Xamariz em Angola.'))
                        ...
                        <div class="aspect-[3/4] overflow-hidden rounded-none bg-gray-100 relative group">
                            @php
                                $ceoPhoto = $ceoMember->photo_path
                                    ? (Str::startsWith($ceoMember->photo_path, ['http://', 'https://'])
                                        ? $ceoMember->photo_path
                                        : asset(ltrim($ceoMember->photo_path, '/')))
                                    : 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&auto=format&fit=crop&q=80';
                            @endphp
                            <img src="{{ $ceoPhoto }}" alt="{{ $ceoMember->name }} - {{ $ceoMember->role }}"
                    ...
                    <div class="lg:col-span-7 reveal delay-100 space-y-6">
                        ...
                        @if ($ceoMember->social_linkedin || $ceoMember->social_twitter || $ceoMember->social_instagram || $ceoMember->email)
                            <div class="flex items-center gap-4 pt-2">
                                <span class="font-sans text-xs font-bold uppercase tracking-widest text-gray-500">REDES
                                    SOCIAIS:</span>
                                @if ($ceoMember->social_linkedin)
                                    <a href="{{ $ceoMember->social_linkedin }}" target="_blank" rel="noopener"
                                        title="LinkedIn"
                                ...
                                @if ($ceoMember->social_twitter)
                                    <a href="{{ $ceoMember->social_twitter }}" target="_blank" rel="noopener"
                                        title="Twitter/X"
                                        class="p-2 text-gray-700 hover:text-white hover:bg-[var(--color-brand-accent)] hover:border-[var(--color-brand-accent)] transition-all duration-200 rounded-none">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                ...
                                @if ($ceoMember->social_instagram)
                                    <a href="{{ $ceoMember->social_instagram }}" target="_blank" rel="noopener"
                                        title="Instagram"
                                        class="p-2 text-gray-700 hover:text-white hover:bg-[var(--color-brand-accent)] hover:border-[var(--color-brand-accent)] transition-all duration-200 rounded-none">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
